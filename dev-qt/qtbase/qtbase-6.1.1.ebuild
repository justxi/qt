...
if [[ ${PV} == *9999* ]]; then
	...
	SRC_URI="https://download.qt.io/official_releases/qt/${PV%.*}/${PV/_/-}/single/${MY_P}.tar.xz"
	KEYWORDS="~amd64"
	S=${WORKDIR}/${MY_P}
fi
...
SLOT=6/$(ver_cut 1-2)
# Qt Modules
IUSE="+clang +concurrent +dbus +gui +network +sql opengl +widgets +xml"
...
PATCHES=(
	"${FILESDIR}/qtbase-6.1.1-qt3d-cmake-add_define_Z_OF.patch"
	"${FILESDIR}/qtbase-6.1.1-qt3d-pri-add_define_Z_OF.patch"
	"${FILESDIR}/qtbase-6.1.1-qt3d-add-exception_ptr.patch"
	"${FILESDIR}/qtbase-6.1.1-qtquick3d-cmake-add_define_Z_OF.patch"
	"${FILESDIR}/qtbase-6.1.1-qtquick3d-add-exception_ptr.patch"
)
...
src_prepare() {
	...
	sed -e "/androiddeployqt/s/^/#DONT/" -e "/androidtestrunner/s/^/#DONT/" \
		-i qtbase/src/tools/CMakeLists.txt || die
}
